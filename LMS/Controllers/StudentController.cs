    ...
    public class StudentController : Controller
    {
        ...
        public IActionResult GetAssignmentsInClass(string subject, int num, string season, int year, string uid)
        {
            var query = from cl in db.Classes
                        ...
                        join s in db.Submitteds on new { A = a.AssignmentId, S = uid } equals new { A = s.AssignmentId, S = s.StudentId }
                        into joined
                        ...
                        select new
                        {
                            ...
                            due = a.DueDate,
                            score = j == null ? null : (ushort?)j.Score
                        };
        ...
        string category, string asgname, string uid, string contents)
        {
            var query = from cl in db.Classes
                        ...
                        join ac in db.AssignmentCategories on cl.ClassId equals ac.ClassId
                        join a in db.Assignments on ac.CategoryId equals a.AssignmentCategoryId
                        where ac.CategoryName == category && a.AssignmentName == asgname
                        select a.AssignmentId;

            int aid = query.FirstOrDefault();
            ...
            var submission = db.Submitteds.FirstOrDefault(s => s.AssignmentId == aid && s.StudentId == uid);

            if (submission == null)
            {
                Submitted newSub = new Submitted
                {
                    AssignmentId = aid,
                    StudentId = uid,
                    Contents = contents,
        ...
        public IActionResult Enroll(string subject, int num, string season, int year, string uid)
        {
            ...
            int cid = (int)classQuery.FirstOrDefault();

            if (cid == 0) return Json(new { success = false });

            bool exists = db.EnrolledIns.Any(e => e.ClassId == cid && e.StudentId == uid);
            if (exists) return Json(new { success = false });

            EnrolledIn enrollment = new EnrolledIn
            {
                ClassId = cid,
                StudentId = uid,
                Grade = ""
        ...
        {
            var grades = db.EnrolledIns.Where(e => e.StudentId == uid && e.Grade != "" && e.Grade != "--")
                                    .Select(e => e.Grade).ToList();

            if (grades.Count == 0) return Json(new { gpa = 0.0 });

            double totalPoints = 0;
            foreach (var g in grades)
            {
                totalPoints += GetPointValue(g);
            }

            return Json(new { gpa = totalPoints / grades.Count });
        }

        // Helper for GPA calculation
        private double GetPointValue(string grade)
        {
            return grade switch
            {
                "A" => 4.0,
