...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private string CreateNewUser(string firstName, string lastName, DateTime DOB, string departmentAbbrev, string role)
        {
            ...
            while (!isUnique)
            {
                uid = "u" + Random.Shared.Next(0, 10_000_000).ToString("D7");

                bool exists = db.Students.Any(s => s.UserId == uid) ||
                              db.Professors.Any(p => p.UserId == uid) ||
                              db.Administrators.Any(a => a.UserId == uid);

                if (!exists) isUnique = true;
            }

            var dateOfBirth = DateOnly.FromDateTime(DOB);
            ...
            else if (role == "Professor")
            {
                db.Professors.Add(new Professor
                {
                    ...
                    LastName = lastName,
                    Dob = dateOfBirth,
                    WorkDeptAbbreviation = departmentAbbrev
                });
            }
            else if (role == "Student")
            {
                db.Students.Add(new Student
                {
                    ...
                    LastName = lastName,
                    Dob = dateOfBirth,
                    MajorDeptAbbreviation = departmentAbbrev
                });
            }
            else
            {
